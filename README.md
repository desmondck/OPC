...
这两组框架及基于框架的原型实现可100%的通过OPC基金会提供的兼容性测试框架。由于OPC DA和OPC AE的框架是做为产品交付的，基于版权问题，我不能将实现放上来，但interface已上传，可资借鉴。
框架的使用方式在test中，可供参考。

本人还设计了一套独立的自动化测试程序，包括OPC DA和OPC AE两部分。
其中，OPC DA的测试框架是基于DSL，当然该DSL是OPC定制的，简单但却很实用。测试框架是独立的、非产品代码，可从本git源获取。

工作中经常遇到的DCOM配置问题想必各位从业者也是必然会遇到的，docs中的《Dcom完全配置手册.doc》应该可以解决各类dcom配置不通的问题

除此之外，各类的OPC文档在网上是极少的，我能搜集到的完整的OPC文档放在material中。注意：这部分文档均为网上采集的文档，本人只是收集，并不对其中的正确性或版权等问题负责。



如若上述资料对各位有所帮助，也算是自己对这个曾经付诸努力行业的一点交代。
